...
const getTopPosts = async (date: string, userId: string) => {
...
const getFromX = () =>
  fetch(
    `https://api.twitter.com/2/users/${USER_ID}/tweets?max_results=5&tweet.fields=created_at&exclude=retweets,replies&expansions=attachments.media_keys&media.fields=url,preview_image_url`,
...
const saveCache = (key: string, data: string) => redis.set(key, data);
