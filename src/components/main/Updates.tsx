  ...
  return (
    <div className="  flex items-center justify-center">
      ...
      <div className="flex flex-col items-center gap-4 w-full p-5">
        ...
        <div className='flex gap-3 w-[10%] mt-2'>
                    <div 
                      className={`w-7 h-6 flex rounded-sm items-center justify-center bg-[#27272A] cursor-pointer hover:bg-[#3f3f46] ${slidePosition >= 0 ? 'opacity-50' : 'opacity-100'}`}
                      onClick={()=>handleBackward({setSlidePosition, visibleCards})}
                    >
                      <IoArrowBackSharp size={14} />
                    </div>
                    <div 
                      className={`w-7 h-6 flex rounded-sm items-center justify-center bg-[#27272A] cursor-pointer hover:bg-[#3f3f46] ${slidePosition > -(updates.length * 5)  ? 'opacity-50' : 'opacity-100'}`}
